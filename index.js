...
const app = express();
// O Render irá fornecer a porta. Não precisamos de definir uma.
...
// Rota que recebe as notificações do PagBank
app.post('/pagbank-notification', (req, res) => {
  console.log('\n--- NOTIFICAÇÃO DO PAGBANK RECEBIDA ---');
  console.log('Conteúdo:', req.body);
  
  const charge = req.body.charges && req.body.charges[0];

  if (!charge) {
    console.log("Notificação recebida, mas sem dados de cobrança.");
  ...
  if (charge.status === 'PAID') {
    ...
    if (parts.length === 2) {
      ...
      console.log("Referência externa em formato inesperado:", charge.reference_id);
    }
  } else {
    console.log(`Status do pagamento não era 'PAID'. Status atual: ${charge.status}`);
  }
